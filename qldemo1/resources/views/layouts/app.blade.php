...
<head>
  ...
  <style>
    ...
    :root{
      --sidebar-bg: #164B71;       /* màu panel chính */
      --sidebar-hover: #1d5a8d;    /* sáng hơn một chút cho hover */
      --sidebar-active: #164B71;   /* trùng hẳn với nền panel */
    }
...
.sidebar form .btn {
  white-space: nowrap;
  overflow: hidden;
  min-height: 40px;                 /* tránh rung chiều cao */
  border-radius: 8px;
}
...
.sidebar form .btn .btn-text {
  ...
  white-space: nowrap;
  overflow: hidden;
  max-width: 220px;                 /* mở: hiển thị đầy đủ */
  opacity: 1;
  transition: max-width .25s ease, opacity .25s ease;
...
(function(){
  ...
  const ICON_MAP = [
    ...
    { k: 'điểm học tập',        icon: 'bi-graph-up' },
  ...
  const HREF_MAP = [
    { test: /admin\/accounts/i,       icon: 'bi-people' },
    { test: /ctct/i,                  icon: 'bi-clipboard-check' },
    { test: /khaothi/i,               icon: 'bi-graph-up' },
